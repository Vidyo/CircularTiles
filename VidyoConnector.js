...
function registerDeviceListeners(vidyoConnector, cameras, microphones, speakers) {
	...
	vidyoConnector.RegisterRemoteCameraEventListener({
		onAdded: function(camera, participant) { /* New camera is available */
				//alert(participant.userId+" added to session");
				$("#renderer2").removeClass("hidden");
				...
				setTimeout(function(){roundify('renderer2');},1750);
			
		},
		onRemoved: function(camera, participant) { /* Existing camera became unavailable */
			/*if () {   
				vidyoConnector.HideView({
					viewId: "Div where camera is renderered"
				});
			}*/
		},
...
function handleDeviceChange(vidyoConnector, cameras, microphones, speakers) {
    ...
    $("#cameras").change(function() {
        ...
        $("#cameras option:selected").each(function() {
			vidyoConnector.HideView({ viewId: "renderer" });
